...
func (h *Handlers) render(w http.ResponseWriter, r *http.Request, tmpl string, variable, data interface{}) error {
	return h.App.Render.Page(w, r, tmpl, variable, data)
}
...
func (h *Handlers) encrypt(text string) (string, error) {
	enc := golaravel.Encryption{Key: []byte(h.App.EncryptionKey)}
